...
# Model Configuration Page
    ...
    with tabs[0]:
        ...
        quantile = st.selectbox("Select Quantile", quantile_values, index=quantile_values.tolist().index(50))
    ...
    with tabs[1]:
        ...
        lr_lags = st.selectbox("Select Target Lags", lr_lags_values, index=lr_lags_values.index(10))
        training_threshold = st.selectbox("Select Training Threshold", training_threshold_values, index=0)
    ...
    with tabs[2]:
        ...
        n_estimators = st.slider("Number of Trees (n_estimators)", min_value=100, max_value=300, value=100, step=100)
        max_depth = st.slider("Maximum Depth (max_depth)", min_value=5, max_value=20, value=10, step=5)
    ...
    global_quantile_metrics = pickle.load(open(f'precomputed_results/qh/global_qh_metrics/{qh_pointer}', 'rb'))
    groups_quantile_metrics = pickle.load(open(f'precomputed_results/qh/groups_qh_metrics/{qh_pointer}', 'rb'))

    # LR    
    lr_pointer = f"{st.session_state['quantile']}_{st.session_state['lr_lags']}_{st.session_state['training_threshold']}.p"
    global_lr_metrics = pickle.load(open(f'precomputed_results/lr/global_lr_metrics/{lr_pointer}', 'rb'))
    groups_lr_metrics = pickle.load(open(f'precomputed_results/lr/groups_lr_metrics/{lr_pointer}', 'rb'))
    lr_results_train = pickle.load(open(f'precomputed_results/lr/lr_results_train/{lr_pointer}', 'rb'))
    ...
    global_rf_metrics = pickle.load(open(f'precomputed_results/rf/global_rf_metrics/{rf_pointer}', 'rb'))
    groups_rf_metrics = pickle.load(open(f'precomputed_results/rf/groups_rf_metrics/{rf_pointer}', 'rb'))
    rf_results_train = pickle.load(open(f'precomputed_results/rf/rf_results_train/{rf_pointer}', 'rb'))
    rf_models = pickle.load(open(f'precomputed_results/rf/rf_models/{rf_pointer}', 'rb'))

    # UB
    ub_pointer = f"{st.session_state['quantile']}.p"
    global_ub_metrics = pickle.load(open(f'precomputed_results/ub/global_ub_metrics/{ub_pointer}', 'rb'))
    groups_ub_metrics = pickle.load(open(f'precomputed_results/ub/groups_ub_metrics/{ub_pointer}', 'rb'))
